...
if (!file.exists("data/README.txt")) {
  ...
  file.copy(from = "data/UCI HAR Dataset/.",
            to = "data/",
            recursive = T)
...
# Reading Data
feat <-
  read.table("data/features.txt", col.names = c("n", "functions"))
act <-
  read.table("data/activity_labels.txt", col.names = c("code", "activity"))

## Test set
sTest <-
  read.table("data/test/subject_test.txt", col.names = "subject")
xTest <-
  read.table("data/test/X_test.txt", col.names = feat$functions)
yTest <-
  read.table("data/test/y_test.txt", col.names = "activitycode")

## Train set
sTrain <-
  read.table("data/train/subject_train.txt", col.names = "subject")
xTrain <-
  read.table("data/train/X_train.txt", col.names = feat$functions)
yTrain <-
  read.table("data/train/y_train.txt", col.names = "activitycode")
...
df <-
  m_df %>% select(subject, activitycode, contains("mean"), contains("std"))
...
names(df) <-
  gsub("-freq()", "Frequency", names(df), ignore.case = TRUE)
